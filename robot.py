#type: ignore
"""Entry point: wires arm simulation, PID control, path planning, and animation into one FRC-style robot loop."""
    ...
    if result is None:
        ...
        if not is_reachable(grid, start, goal):
            ...
            if path is None:
                ...
                robot = Robot()
                robot.robotInit(interp_smoothed)
                animation.animate_path(interp_smoothed, interp_smoothed, obstacles, grid,
                                       title='smoothed — PID', robot=robot)
                plt.show()
